...
export default function TourDetailClient({ tour }: TourDetailClientProps) {
  ...
  // Get translated description, fallback to tour data
  const description = tItems.has(`${tour.slug}.description`)
    ? tItems(`${tour.slug}.description`)
    : tour.description;
  ...
  if (showSuccess) {
    return <SuccessConfirmation tripRef={tripRef} whatsappUrl={whatsappUrl} />;
  }
    ...
    <>
      ...
      {showMobileForm && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute bottom-0 left-0 right-0 max-h-[90vh] overflow-y-auto rounded-t-3xl bg-background p-6 shadow-2xl animate-in slide-in-from-bottom duration-300">
            ...
            <TourBookingForm tour={tour} onSuccess={handleSuccess} />
          </div>
        </div>
      )}
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-10">
          ...
          {/* Excursions */}
          {(tour.excursions.enRoute.length > 0 ||
            tour.excursions.atDestination.length > 0) && (
            <section id="excursions">
              ...
              {tour.excursions.enRoute.length > 0 && (
                <div className="mt-6">
                  ...
                  <div className="mt-3 space-y-3">
                    {tour.excursions.enRoute.map((excursion) => (
                      <div
                        key={excursion.name}
              ...
              {tour.excursions.atDestination.length > 0 && (
                <div className="mt-6">
                  ...
                  <div className="mt-3 space-y-3">
                    {tour.excursions.atDestination.map((excursion) => (
                      <div
                        key={excursion.name}
