...
export default function PackageBookingForm({ package: pkg, onSuccess }: PackageBookingFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = `Hi! I'd like to book the ${pkg.title} package.
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-xs text-[color-mix(in_oklab,var(--foreground)_60%,transparent)]">
          {t("travelDatesNote", { days: pkg.duration })}
        </p>
